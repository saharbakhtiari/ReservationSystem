...
namespace Application_Backend.EmailNotifications.Commands.CheckEmailNotification;

public class CheckRuleSmsNotifyCommandHandler : IRequestHandler<CheckRuleSmsNotifyCommand, bool>
{
    private readonly IStringLocalizer _localizer;
    private readonly ICurrentUserService _currentUserService;
    public CheckRuleSmsNotifyCommandHandler(IStringLocalizer localizer, ICurrentUserService currentUserService)
    {
        _localizer = localizer;
        _currentUserService = currentUserService;
    }

    public async Task<bool> Handle(CheckRuleSmsNotifyCommand request, CancellationToken cancellationToken)
    {
        ...
        CheckSmsNotify checkNotify = new()
        {
            PhoneNumber = profile.PhoneNumber,
            Key = $"{NotificationStatics.RuleKey}{request.RuleId}"
        };
        return await checkNotify.SendAsync(cancellationToken);
