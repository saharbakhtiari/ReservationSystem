...
namespace Domain.UnitOfWork.Uow
{
    public class UnitOfWorkManager : IUnitOfWorkManager//, ISingletonDependency
    {
        ...
        public IUnitOfWork Begin(SedUnitOfWorkOptions options, bool requiresNew = false)
        {
        ...
        public void BeginReserved(string reservationName, SedUnitOfWorkOptions options)
        {
            if (!TryBeginReserved(reservationName, options))
            {
                throw new SedException($"Could not find a reserved unit of work with reservation name: {reservationName}");
            }
        }

        public bool TryBeginReserved(string reservationName, SedUnitOfWorkOptions options)
        {
            Check.NotNull(reservationName, nameof(reservationName));
