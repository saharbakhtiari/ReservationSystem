...
{
    public class InvoiceRepository : GenericRepository<Invoice, long>, IInvoiceRepository
    {
        public InvoiceRepository(IDbContextProvider<ApplicationDbContext> dbContextProvider) : base(dbContextProvider)
        {
        }

        public Task<Invoice> GetAsync(long id, CancellationToken cancellationToken)
        {
        ...
        public Task<PagedList<TOutput>> GetFilteredAsync<TOutput>( string filter, string sort, int PageNumber, int PageSize, CancellationToken cancellationToken)
        {
            ...
            return GetAllAsQueryable()
                .Where(x => !x.IsDeleted)
                .WhereIf(filter.IsNullOrWhiteSpace().Not(), r => r.InvoiceNumber.Contains(filter))
